        ...
        <div class="container">
          <div class="row">
            ...
            <div class="col-md-9 bg-white padding-2">
              ...
              // $_SESSION[id_company]
              if ($result->num_rows > 0) {
                $jobs = array(); // Array to store job titles

                while ($row = $result->fetch_assoc()) {
                  // Store details under job titles
                  if (!isset($jobs[$row['jobtitle']])) {
                    $jobs[$row['jobtitle']] = array();
                  }

                  $jobs[$row['jobtitle']][] = $row;
                }

                foreach ($jobs as $jobtitle => $applicants) {
              ?>
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                      ...
                      ?>
                        <li>
                          <a href="user-application.php?id=<?php echo $applicant['id_user']; ?>&id_jobpost=<?php echo $applicant['id_jobpost']; ?>">
                            <?php echo $applicant['firstname'] . ' ' . $applicant['lastname']; ?>
                          </a>
                          <?php
                          if ($applicant['status'] == 0) {
                            echo '<span class="text-orange">Placed</span>';
                          } else if ($applicant['status'] == 1) {
                            echo '<span class="text-red">Rejected</span>';
                          } else if ($applicant['status'] == 2) {
                            echo '<span class="text-green">Applied</span>';
                          }
                          ?>
                        </li>
                      <?php } ?>
